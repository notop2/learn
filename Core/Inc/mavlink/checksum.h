#pragma once

#include <stdint.h>

static const uint8_t mavlink_crc16_table[256] = {
    
};

static inline uint16_t crc_calculate(const uint8_t *buf, uint16_t len)
{
    uint16_t crc = 0xFFFF;
    for (uint16_t i = 0; i < len; i++) {
        crc = (crc >> 8) ^ mavlink_crc16_table[(crc ^ buf[i]) & 0xFF];
    }
    return crc;
}

static inline void crc_accumulate(uint8_t data, uint16_t *crc)
{
    *crc = (*crc >> 8) ^ mavlink_crc16_table[(*crc ^ data) & 0xFF];
}

static inline void crc_accumulate_buffer(uint16_t *crc, const char *buf, uint16_t len)
{
    for (uint16_t i = 0; i < len; i++) {
        *crc = (*crc >> 8) ^ mavlink_crc16_table[(*crc ^ (uint8_t)buf[i]) & 0xFF];
    }
}
